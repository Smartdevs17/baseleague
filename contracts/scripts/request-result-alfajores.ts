...
async function main() {
	console.log('📡 Requesting Match Result from Chainlink Functions (Celo Alfajores)\n')

	// Get contract addresses
	const resultsConsumerAddress = process.env.RESULTS_CONSUMER_ADDRESS_ALFAJORES || process.env.RESULTS_CONSUMER_ADDRESS
	if (!resultsConsumerAddress) {
		throw new Error('RESULTS_CONSUMER_ADDRESS_ALFAJORES not set in .env')
	}
	...
	console.log('  Is caller owner?', signer.address.toLowerCase() === contractOwner.toLowerCase())
	console.log('')

	if (subscriptionId === 0n) {
		throw new Error('Subscription ID not set! Set it first with: npm run set:subscription:alfajores')
	}

	if (!isAuthorized && signer.address.toLowerCase() !== contractOwner.toLowerCase()) {
	...
	try {
		...
		if (eventLog) {
			...
			if (!fulfilled) {
				...
				console.log('   Check status later with: npm run check:result:alfajores')
			}
		} else {
