...
import { useToken, useMatches, useMatch } from '@/hooks/useContracts';
...
const ContractExample = () => {
  ...
  const handleCreateMatch = async () => {
    try {
      const amount = BigInt(stakeAmount) * BigInt(10 ** 18);
      await match.createMatch('fixture-123', prediction, amount);
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <select
              id="prediction"
              value={prediction}
              onChange={(e) => setPrediction(Number(e.target.value) as Prediction)}
              className="w-full p-2 border rounded-md"
            >
              <option value={Prediction.HOME}>Home Win</option>
              <option value={Prediction.DRAW}>Draw</option>
              <option value={Prediction.AWAY}>Away Win</option>
            </select>
          </div>
